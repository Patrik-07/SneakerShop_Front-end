import { Card } from "../card/card";
import { CardContainer } from "../card/card-container";

const exampleShoes = [
  {
    name: "Nike Luka 1",
    price: { original: 119, withDiscount: 99 },
    tags: { isNew: true },
    styles: [
      {
        color: "bg-[#FFA500]",
        image: "src\\assets\\shoes\\nike-luka-1\\nike_luka_1_v01.png",
      },
      {
        color: "bg-[#FF0000]",
        image: "src\\assets\\shoes\\nike-luka-1\\nike_luka_1_v02.png",
      },
      {
        color: "bg-[#0000FF]",
        image: "src\\assets\\shoes\\nike-luka-1\\nike_luka_1_v03.png",
      },
    ],
  },
  {
    name: "Nike Infinity Pro 2",
    price: { original: 179 },
    styles: [
      {
        color: "bg-[#FF0000]",
        image:
          "src\\assets\\shoes\\nike-infinity-pro-2\\nike_infinity_pro_2_v01.png",
      },
      {
        color: "bg-[#0000FF]",
        image:
          "src\\assets\\shoes\\nike-infinity-pro-2\\nike_infinity_pro_2_v04.png",
      },
      {
        color: "bg-[#FFFFFF]",
        image:
          "src\\assets\\shoes\\nike-infinity-pro-2\\nike_infinity_pro_2_v02.png",
      },
      {
        color: "bg-[#000000]",
        image:
          "src\\assets\\shoes\\nike-infinity-pro-2\\nike_infinity_pro_2_v03.png",
      },
    ],
  },
  {
    name: "Nike React Infinity 3",
    price: { original: 129 },
    tags: { isNew: true, isHot: true },
    styles: [
      {
        image:
          "src\\assets\\shoes\\nike-react-infinity-3\\nike_react_infinity_3_v01.png",
      },
    ],
  },
  {
    name: "Nike React Infinity 3",
    price: { original: 129 },
    styles: [
      {
        image:
          "src\\assets\\shoes\\nike-react-infinity-3\\nike_react_infinity_3_v01.png",
      },
    ],
  },
];

export const ExampleCardContainer = () => {
  return (
    <CardContainer>
      <Card shoeData={exampleShoes[0]} color="bg-[#7BE498]" />
      <Card shoeData={exampleShoes[1]} color="bg-[#F5E6FF]" />
      <Card shoeData={exampleShoes[2]} color="bg-[#F7C29B]" />
      <Card shoeData={exampleShoes[3]} color="bg-[#68EAEB]" />
    </CardContainer>
  );
};
